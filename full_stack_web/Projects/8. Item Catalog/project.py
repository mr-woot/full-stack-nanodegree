...
def newSong(genre_id):
    ...
    genre = session.query(Genre).filter_by(id = genre_id).one()
    if request.method == 'POST':
        newSong = Song(genre_id = genre_id,
...
def editSong(genre_id, song_id):
    ...
    if request.method == 'POST':
        ...
        if request.form['author']:
            editedSong.author = request.form['author']
        ...
        return render_template('editSong.html', genre_id = genre_id, song_id = song_id, item = editedSong)
...
def deleteSong(genre_id, song_id):
    if 'username' not in login_session:
        return redirect('/login')
    genre = session.query(Genre).filter_by(id = genre_id).one()
    songToBeDelete = session.query(Song).filter_by(id = song_id).one()
    if songToBeDelete.user_id != login_session['user_id']:
        return """<script>(function() {alert("not authorized");})();</script>"""
    if request.method == 'POST':
        session.delete(songToBeDelete)
        session.commit()
        flash('Song successfully deleted')
        return redirect(url_for('showSongs', genre_id = genre_id))
    else:
        return render_template('deleteSong.html', item = songToBeDelete)


if __name__ == '__main__':
    app.secret_key = "eagerly waiting for someone...!"
    app.debug = True
