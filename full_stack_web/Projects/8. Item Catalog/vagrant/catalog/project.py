...
def showSongs(genre_id):
    ...
    if 'username' not in login_session or creator.id \
        ...
        return render_template('songs.html', songs=songs, genre=genre,
                               genres=genres, creator=creator)
...
def newSong(genre_id):
    """ Creates a song from the respective genre
    """
    genre = session.query(Genre).filter_by(id=genre_id).one()
    if request.method == 'POST':
        newSong = Song(
...
def editSong(genre_id, song_id):
    """ Edits a song from the respective genre
    """
    ...
    if request.method == 'POST':
        ...
        return render_template('edit-song.html', genre_id=genre_id,
                               song_id=song_id, item=editedSong)


# Delete a song

@login_required
@app.route('/genre/<int:genre_id>/songs/<int:song_id>/delete',
           methods=['GET', 'POST'])
def deleteSong(genre_id, song_id):
    """ Deletes a song from the respective genre
    """
    genre = session.query(Genre).filter_by(id=genre_id).one()
